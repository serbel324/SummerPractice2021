...
std::istream& operator>>(std::istream& in, FieldZp& r)
{
    ...
    r.set_value((n + FieldZp::orderLL) % FieldZp::order);
    return in;
}
...
{
public:
    Polynom() : coeff(0) { reduce(); }
    Polynom(FieldZp f) : coeff(1, f) { reduce(); }
...
{
    while (coeff.size() > 0 && coeff.back() == 0)
    {
        coeff.pop_back();
    }
}
...
int main_encode()
{
    ...
    int nn = f.size();
    Number n = f.size();
    Number q = p.power(n - 1);

    Polynom::set_gen(f);
    Polynom::set_order(q);

    std::getline(std::cin, inp);
    Polynom g = read_polynom(inp);
    std::getline(std::cin, inp);
    Polynom k = read_polynom(inp);

    std::getline(std::cin, inp);
    std::vector<Number> num;
    for (int i = 0; i < inp.size(); ++i)
    {
        num.push_back(char_to_number_fixed(inp[i]));
    }

    auto num1 = change_base(num, 64, p);

    int i = 0;
    for (; i < num1.size(); i += nn - 1)
    {
        std::vector<FieldZp> v;
        for (int j = 0; j < n - 1; ++j)
        {
            if (i + j == num1.size())
                break;
            v.push_back(FieldZp(num1[i + j]));
        }
        Number b = get_random(1, p);
        Polynom pol(v);
        Polynom elem = elgamal_encode(v, g, k, b);
        std::cout << g.power(b) << std::endl << elem << std::endl;
    }
